...
$buscar = isset($_GET['buscarActivos']) ? $_GET['buscarActivos'] : '';
...
if ($envio->num_rows > 0) {
    ...
    while ($row = $envio->fetch_assoc()) {
        ...
        echo "<p>$" . $row["precio"] . "</p>";
        echo "<button class='deshabilitar-producto' data-id='" . $row["ID"] . "' onclick='deshabilitarProducto(this, " . $row["ID"] . ")'><img class='eliminar-carrito' src='img/menos.png'></button>";
    ...
    for ($i = 1; $i <= $totalPaginas; $i++) {
        echo "<a href='?pagina=$i&buscarActivos=$buscar'>$i</a>";
    }
    echo "</div>";
} else {
...
<script>
function deshabilitarProducto(boton, idProducto) {
    fetch('deshabilita_producto.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'id_producto=' + idProducto + '&accion=deshabilita',
    })
    ...
    .then(data => {
        if (data.exito) {
            ...
            console.error('Error al deshabilitar el producto:', data.mensaje);
        }
    })
    .catch(error => {
...
function habilitarProducto(boton, idProducto) {
    fetch('habilitar_producto.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'id_producto=' + idProducto + '&accion=habilita',
    })
    .then(response => response.json())
    .then(data => {
        if (data.exito) {
